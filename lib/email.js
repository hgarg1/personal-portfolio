...
async function sendResponseEmail(recipientEmail, recipientName, subject, responseMessage) {
  const transporter = getTransporter();
  
  if (!transporter) {
    console.log(`\n==================================================`);
    console.log(`✉️  [MOCK EMAIL RESPONSE SENT TO: ${recipientEmail}]`);
    console.log(`Subject: ${subject}`);
    console.log(`Message:\n${responseMessage}`);
    console.log(`==================================================\n`);
    return false;
  ...
  const mailOptions = {
    from: `"Harshit Garg" <${process.env.GMAIL_USER}>`,
    to: recipientEmail,
    subject: subject,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 25px; border: 1px solid #e5e7eb; border-radius: 8px; background-color: #ffffff; color: #374151;">
        <div style="font-size: 16px; line-height: 1.6; white-space: pre-wrap; font-family: inherit;">
          ${responseMessage}
        </div>
        <hr style="border: 0; border-top: 1px solid #f3f4f6; margin: 20px 0;" />
        <p style="color: #9ca3af; font-size: 12px; text-align: center;">This message was sent from Harshit Garg's Administration Portal.</p>
      </div>
    `,
  };
  ...
  try {
    await transporter.sendMail(mailOptions);
    console.log(`📨 Real email response sent to ${recipientEmail}`);
    return true;
  } catch (error) {
    console.error(`❌ Failed to send response email to ${recipientEmail}:`, error);
    // Fallback log
    console.log(`\n==================================================`);
    console.log(`✉️  [FALLBACK MOCK EMAIL RESPONSE TO: ${recipientEmail}]`);
    console.log(`Subject: ${subject}`);
    console.log(`Message:\n${responseMessage}`);
    console.log(`==================================================\n`);
    return false;
